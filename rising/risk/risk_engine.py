...
class RiskEngine:
    def __init__(self, min_liquidity_usd: float, min_volume_5m_usd: float, max_pump_5m_pct: float) -> None:
    ...
    def score(self, snapshot: MarketSnapshot) -> RiskResult:
        ...
        if snapshot.pair_address is None:
            return RiskResult(score=100, blocked=True, reasons=["No Solana pair found on DexScreener"])
        ...
        if snapshot.liquidity_usd is None or snapshot.liquidity_usd < self.min_liquidity_usd:
            score += 45
            reasons.append(f"Low liquidity: {snapshot.liquidity_usd}")

        if snapshot.volume_5m_usd is None or snapshot.volume_5m_usd < self.min_volume_5m_usd:
            score += 15
            reasons.append(f"Low 5m volume: {snapshot.volume_5m_usd}")
        ...
        return RiskResult(score=min(score, 100), blocked=blocked, reasons=reasons or ["Risk checks passed"])
